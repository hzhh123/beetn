/*
 * Generated by the Jasper component of Apache Tomcat
 * Version: jetty/9.3.10.v20160621
 * Generated at: 2018-03-02 09:37:21 UTC
 * Note: The last modified time of this file was set to
 *       the last modified time of the source file after
 *       generation to assist with modification tracking.
 */
package org.apache.jsp.jsp.org;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;

public final class organizationList_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent,
                 org.apache.jasper.runtime.JspSourceImports {

  private static final javax.servlet.jsp.JspFactory _jspxFactory =
          javax.servlet.jsp.JspFactory.getDefaultFactory();

  private static java.util.Map<java.lang.String,java.lang.Long> _jspx_dependants;

  static {
    _jspx_dependants = new java.util.HashMap<java.lang.String,java.lang.Long>(7);
    _jspx_dependants.put("/common/head.jsp", Long.valueOf(1516501887843L));
    _jspx_dependants.put("/common/global.jsp", Long.valueOf(1516501887843L));
    _jspx_dependants.put("file:/D:/soft/maven/maven-repository/javax/servlet/jstl/1.2/jstl-1.2.jar", Long.valueOf(1503472483648L));
    _jspx_dependants.put("jar:file:/D:/soft/maven/maven-repository/javax/servlet/jstl/1.2/jstl-1.2.jar!/META-INF/fmt.tld", Long.valueOf(1153356282000L));
    _jspx_dependants.put("jar:file:/D:/soft/maven/maven-repository/javax/servlet/jstl/1.2/jstl-1.2.jar!/META-INF/c.tld", Long.valueOf(1153356282000L));
    _jspx_dependants.put("jar:file:/D:/soft/maven/maven-repository/javax/servlet/jstl/1.2/jstl-1.2.jar!/META-INF/fn.tld", Long.valueOf(1153356282000L));
    _jspx_dependants.put("/common/foot.jsp", Long.valueOf(1516501887843L));
  }

  private static final java.util.Set<java.lang.String> _jspx_imports_packages;

  private static final java.util.Set<java.lang.String> _jspx_imports_classes;

  static {
    _jspx_imports_packages = new java.util.HashSet<>();
    _jspx_imports_packages.add("javax.servlet");
    _jspx_imports_packages.add("javax.servlet.http");
    _jspx_imports_packages.add("javax.servlet.jsp");
    _jspx_imports_classes = null;
  }

  private org.apache.jasper.runtime.TagHandlerPool _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody;

  private volatile javax.el.ExpressionFactory _el_expressionfactory;
  private volatile org.apache.tomcat.InstanceManager _jsp_instancemanager;

  public java.util.Map<java.lang.String,java.lang.Long> getDependants() {
    return _jspx_dependants;
  }

  public java.util.Set<java.lang.String> getPackageImports() {
    return _jspx_imports_packages;
  }

  public java.util.Set<java.lang.String> getClassImports() {
    return _jspx_imports_classes;
  }

  public javax.el.ExpressionFactory _jsp_getExpressionFactory() {
    if (_el_expressionfactory == null) {
      synchronized (this) {
        if (_el_expressionfactory == null) {
          _el_expressionfactory = _jspxFactory.getJspApplicationContext(getServletConfig().getServletContext()).getExpressionFactory();
        }
      }
    }
    return _el_expressionfactory;
  }

  public org.apache.tomcat.InstanceManager _jsp_getInstanceManager() {
    if (_jsp_instancemanager == null) {
      synchronized (this) {
        if (_jsp_instancemanager == null) {
          _jsp_instancemanager = org.apache.jasper.runtime.InstanceManagerFactory.getInstanceManager(getServletConfig());
        }
      }
    }
    return _jsp_instancemanager;
  }

  public void _jspInit() {
    _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody = org.apache.jasper.runtime.TagHandlerPool.getTagHandlerPool(getServletConfig());
  }

  public void _jspDestroy() {
    _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.release();
  }

  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
        throws java.io.IOException, javax.servlet.ServletException {

final java.lang.String _jspx_method = request.getMethod();
if (!"GET".equals(_jspx_method) && !"POST".equals(_jspx_method) && !"HEAD".equals(_jspx_method) && !javax.servlet.DispatcherType.ERROR.equals(request.getDispatcherType())) {
response.sendError(HttpServletResponse.SC_METHOD_NOT_ALLOWED, "JSPs only permit GET POST or HEAD");
return;
}

    final javax.servlet.jsp.PageContext pageContext;
    javax.servlet.http.HttpSession session = null;
    final javax.servlet.ServletContext application;
    final javax.servlet.ServletConfig config;
    javax.servlet.jsp.JspWriter out = null;
    final java.lang.Object page = this;
    javax.servlet.jsp.JspWriter _jspx_out = null;
    javax.servlet.jsp.PageContext _jspx_page_context = null;


    try {
      response.setContentType("text/html;charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;

      if (_jspx_meth_c_005fset_005f0(_jspx_page_context))
        return;
      if (_jspx_meth_c_005fset_005f1(_jspx_page_context))
        return;
      if (_jspx_meth_c_005fset_005f2(_jspx_page_context))
        return;
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("    var basePath = \"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath }", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("\";\r\n");
      out.write("    window.UEDITOR_HOME_URL = \"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath }", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/ueditor/\";\r\n");
      out.write("    window.UEDITOR_SERVER_URL = \"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath }", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/ueditor\";\r\n");
      out.write("</script>");
      out.write("<!DOCTYPE html>\r\n");
      out.write("<html lang=\"en\">\r\n");
      out.write("<head>\r\n");
      out.write("    <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge,chrome=1\" />\r\n");
      out.write("    <meta charset=\"utf-8\" />\r\n");
      out.write("    <title>主页</title>\r\n");
      out.write("\r\n");
      out.write("    <meta name=\"description\" content=\"overview &amp; stats\" />\r\n");
      out.write("    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0\" />\r\n");
      out.write("    ");
      out.write("<link  rel=\"shortcut icon\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/favicon/favicon.ico\">\r\n");
      out.write("<!-- bootstrap & fontawesome -->\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/bootstrap.css\" />\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/font-awesome.css\" />\r\n");
      out.write("\r\n");
      out.write("<!-- page specific plugin styles -->\r\n");
      out.write("\r\n");
      out.write("<!-- text fonts -->\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/ace-fonts.css\" />\r\n");
      out.write("\r\n");
      out.write("<!-- ace styles -->\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/ace.css\" class=\"ace-main-stylesheet\" id=\"main-ace-style\" />\r\n");
      out.write("\r\n");
      out.write("<!--[if lte IE 9]>\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/ace-part2.css\" class=\"ace-main-stylesheet\" />\r\n");
      out.write("<![endif]-->\r\n");
      out.write("\r\n");
      out.write("<!--[if lte IE 9]>\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/ace-ie.css\" />\r\n");
      out.write("<![endif]-->\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/css/aceadmin-style.css\">\r\n");
      out.write("<!-- inline styles related to this page -->\r\n");
      out.write("\r\n");
      out.write("<!-- ace settings handler -->\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace-extra.js\"></script>\r\n");
      out.write("    <link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/layui/css/layui.css\">\r\n");
      out.write("<!-- HTML5shiv and Respond.js for IE8 to support HTML5 elements and media queries -->\r\n");
      out.write("    <link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${staticPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/plugin/jstree/dist/themes/default/style.css\">\r\n");
      out.write("<!--[if lte IE 8]>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/html5shiv.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/respond.js\"></script>\r\n");
      out.write("<![endif]-->\r\n");
      out.write("<style type=\"text/css\">\r\n");
      out.write("    .ace-nav{\r\n");
      out.write("        height: 45px!important;\r\n");
      out.write("    }\r\n");
      out.write("    .sidebar-shortcuts .btn{\r\n");
      out.write("        padding: 6px!important;\r\n");
      out.write("    }\r\n");
      out.write("    .breadcrumb{\r\n");
      out.write("        padding-top: 6px;\r\n");
      out.write("    }\r\n");
      out.write("\r\n");
      out.write("    .main-content{\r\n");
      out.write("        min-height: calc(100% - 45px)!important;\r\n");
      out.write("    }\r\n");
      out.write("\r\n");
      out.write("</style>\r\n");
      out.write("<style type=\"text/css\">\r\n");
      out.write("     .ace-nav{\r\n");
      out.write("            height: 45px!important;\r\n");
      out.write("        }\r\n");
      out.write("        .sidebar-shortcuts .btn{\r\n");
      out.write("            padding: 6px!important;\r\n");
      out.write("        }\r\n");
      out.write("        .breadcrumb{\r\n");
      out.write("            padding-top: 6px;\r\n");
      out.write("        }\r\n");
      out.write("        .main-content{\r\n");
      out.write("            min-height: calc(100% - 45px)!important;\r\n");
      out.write("        }\r\n");
      out.write("        .laytable-cell-checkbox{\r\n");
      out.write("            padding-top: 5px;\r\n");
      out.write("        }\r\n");
      out.write("        @media(max-width: 768px) {\r\n");
      out.write("            .layui-layer-iframe{\r\n");
      out.write("                width:100%!important;\r\n");
      out.write("                height:100%!important;\r\n");
      out.write("            }\r\n");
      out.write("        }\r\n");
      out.write("    </style>\r\n");
      out.write("</head>\r\n");
      out.write("\r\n");
      out.write("<body class=\"no-skin\">\r\n");
      out.write("<!-- #section:basics/navbar.layout -->\r\n");
      org.apache.jasper.runtime.JspRuntimeLibrary.include(request, response, "/common/navbar.jsp", out, false);
      out.write("<!-- /section:basics/navbar.layout -->\r\n");
      out.write("<div class=\"main-container\" id=\"main-container\">\r\n");
      out.write("    <script type=\"text/javascript\">\r\n");
      out.write("        try{ace.settings.check('main-container' , 'fixed')}catch(e){}\r\n");
      out.write("    </script>\r\n");
      out.write("\r\n");
      out.write("    <!-- #section:basics/sidebar -->\r\n");
      out.write("    <div id=\"sidebar\" class=\"sidebar                  responsive sidebar-fixed  \"  data-scroll-to-active=\"true\" data-include-shortcuts=\"true\" data-smooth-scroll=\"150\">\r\n");
      out.write("        <script type=\"text/javascrH-ui.adminipt\">\r\n");
      out.write("\t\t\t\t\ttry{ace.settings.check('sidebar' , 'fixed')}catch(e){}\r\n");
      out.write("\t\t\t\t</script>\r\n");
      out.write("        <div class=\"sidebar-shortcuts\" id=\"sidebar-shortcuts\" >\r\n");
      out.write("            <div class=\"sidebar-shortcuts-large\" id=\"sidebar-shortcuts-large\">\r\n");
      out.write("                <button class=\"btn btn-success\">\r\n");
      out.write("                    <i class=\"ace-icon fa fa-signal\"></i>\r\n");
      out.write("                </button>\r\n");
      out.write("\r\n");
      out.write("                <button class=\"btn btn-info\">\r\n");
      out.write("                    <i class=\"ace-icon fa fa-pencil\"></i>\r\n");
      out.write("                </button>\r\n");
      out.write("\r\n");
      out.write("                <!-- #section:basics/sidebar.layout.shortcuts -->\r\n");
      out.write("                <button class=\"btn btn-warning\">\r\n");
      out.write("                    <i class=\"ace-icon fa fa-users\"></i>\r\n");
      out.write("                </button>\r\n");
      out.write("\r\n");
      out.write("                <button class=\"btn btn-danger\">\r\n");
      out.write("                    <i class=\"ace-icon fa fa-cogs\"></i>\r\n");
      out.write("                </button>\r\n");
      out.write("\r\n");
      out.write("                <!-- /section:basics/sidebar.layout.shortcuts -->\r\n");
      out.write("            </div>\r\n");
      out.write("\r\n");
      out.write("            <div class=\"sidebar-shortcuts-mini\" id=\"sidebar-shortcuts-mini\">\r\n");
      out.write("                <span class=\"btn btn-success\"></span>\r\n");
      out.write("\r\n");
      out.write("                <span class=\"btn btn-info\"></span>\r\n");
      out.write("\r\n");
      out.write("                <span class=\"btn btn-warning\"></span>\r\n");
      out.write("\r\n");
      out.write("                <span class=\"btn btn-danger\"></span>\r\n");
      out.write("            </div>\r\n");
      out.write("        </div><!-- /.sidebar-shortcuts -->\r\n");
      out.write("        ");
      org.apache.jasper.runtime.JspRuntimeLibrary.include(request, response, "/common/sidebar.jsp", out, false);
      out.write("<!-- #section:basics/sidebar.layout.minimize -->\r\n");
      out.write("        <div class=\"sidebar-toggle sidebar-collapse\" id=\"sidebar-collapse\">\r\n");
      out.write("            <i class=\"ace-icon fa fa-angle-double-left\" data-icon1=\"ace-icon fa fa-angle-double-left\" data-icon2=\"ace-icon fa fa-angle-double-right\"></i>\r\n");
      out.write("        </div>\r\n");
      out.write("\r\n");
      out.write("        <!-- /section:basics/sidebar.layout.minimize -->\r\n");
      out.write("        <script type=\"text/javascript\">\r\n");
      out.write("            try{ace.settings.check('sidebar' , 'collapsed')}catch(e){}\r\n");
      out.write("        </script>\r\n");
      out.write("    </div>\r\n");
      out.write("\r\n");
      out.write("    <!-- /section:basics/sidebar -->\r\n");
      out.write("    <div class=\"main-content\">\r\n");
      out.write("        <div class=\"main-content-inner\">\r\n");
      out.write("            <!-- #section:basics/content.breadcrumbs -->\r\n");
      out.write("            <div class=\"breadcrumbs breadcrumbs-fixed\" id=\"breadcrumbs\">\r\n");
      out.write("                <script type=\"text/javascript\">\r\n");
      out.write("                    try{ace.settings.check('breadcrumbs' , 'fixed')}catch(e){}\r\n");
      out.write("                </script>\r\n");
      out.write("\r\n");
      out.write("                <ul class=\"breadcrumb\">\r\n");
      out.write("                    <li>\r\n");
      out.write("                        <i class=\"ace-icon fa fa-home home-icon\"></i>\r\n");
      out.write("                        <a href=\"/index\">主页</a>\r\n");
      out.write("                    </li>\r\n");
      out.write("                    <li class=\"active\">机构管理</li>\r\n");
      out.write("                </ul><!-- /.breadcrumb -->\r\n");
      out.write("\r\n");
      out.write("                <!-- #section:basics/content.searchbox -->\r\n");
      out.write("                <div class=\"nav-search\" id=\"nav-search\">\r\n");
      out.write("                    <form class=\"form-search\">\r\n");
      out.write("\t\t\t\t\t\t\t\t<span class=\"input-icon\">\r\n");
      out.write("\t\t\t\t\t\t\t\t\t<input type=\"text\" placeholder=\"Search ...\" class=\"nav-search-input\" id=\"nav-search-input\" autocomplete=\"off\" />\r\n");
      out.write("\t\t\t\t\t\t\t\t\t<i class=\"ace-icon fa fa-search nav-search-icon\"></i>\r\n");
      out.write("\t\t\t\t\t\t\t\t</span>\r\n");
      out.write("                    </form>\r\n");
      out.write("                </div><!-- /.nav-search -->\r\n");
      out.write("\r\n");
      out.write("                <!-- /section:basics/content.searchbox -->\r\n");
      out.write("            </div>\r\n");
      out.write("\r\n");
      out.write("            <!-- /section:basics/content.breadcrumbs -->\r\n");
      out.write("            <div class=\"page-content\">\r\n");
      out.write("                <!-- #section:settings.box -->\r\n");
      out.write("                <div class=\"ace-settings-container\" id=\"ace-settings-container\">\r\n");
      out.write("                    <div class=\"btn btn-app btn-xs btn-warning ace-settings-btn\" id=\"ace-settings-btn\">\r\n");
      out.write("                        <i class=\"ace-icon fa fa-cog bigger-130\"></i>\r\n");
      out.write("                    </div>\r\n");
      out.write("\r\n");
      out.write("                    <div class=\"ace-settings-box clearfix\" id=\"ace-settings-box\">\r\n");
      out.write("                        <div class=\"pull-left width-50\">\r\n");
      out.write("                            <!-- #section:settings.skins -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <div class=\"pull-left\">\r\n");
      out.write("                                    <select id=\"skin-colorpicker\" class=\"hide\">\r\n");
      out.write("                                        <option data-skin=\"no-skin\" value=\"#438EB9\">#438EB9</option>\r\n");
      out.write("                                        <option data-skin=\"skin-1\" value=\"#222A2D\">#222A2D</option>\r\n");
      out.write("                                        <option data-skin=\"skin-2\" value=\"#C6487E\">#C6487E</option>\r\n");
      out.write("                                        <option data-skin=\"skin-3\" value=\"#D0D0D0\">#D0D0D0</option>\r\n");
      out.write("                                    </select>\r\n");
      out.write("                                </div>\r\n");
      out.write("                                <span>&nbsp; Choose Skin</span>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.skins -->\r\n");
      out.write("\r\n");
      out.write("                            <!-- #section:settings.navbar -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-navbar\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-navbar\"> Fixed Navbar</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.navbar -->\r\n");
      out.write("\r\n");
      out.write("                            <!-- #section:settings.sidebar -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-sidebar\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-sidebar\"> Fixed Sidebar</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.sidebar -->\r\n");
      out.write("\r\n");
      out.write("                            <!-- #section:settings.breadcrumbs -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-breadcrumbs\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-breadcrumbs\"> Fixed Breadcrumbs</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.breadcrumbs -->\r\n");
      out.write("\r\n");
      out.write("                            <!-- #section:settings.rtl -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-rtl\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-rtl\"> Right To Left (rtl)</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.rtl -->\r\n");
      out.write("\r\n");
      out.write("                            <!-- #section:settings.container -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-add-container\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-add-container\">\r\n");
      out.write("                                    Inside\r\n");
      out.write("                                    <b>.container</b>\r\n");
      out.write("                                </label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:settings.container -->\r\n");
      out.write("                        </div><!-- /.pull-left -->\r\n");
      out.write("\r\n");
      out.write("                        <div class=\"pull-left width-50\">\r\n");
      out.write("                            <!-- #section:basics/sidebar.options -->\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-hover\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-hover\"> Submenu on Hover</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-compact\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-compact\"> Compact Sidebar</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <div class=\"ace-settings-item\">\r\n");
      out.write("                                <input type=\"checkbox\" class=\"ace ace-checkbox-2\" id=\"ace-settings-highlight\" />\r\n");
      out.write("                                <label class=\"lbl\" for=\"ace-settings-highlight\"> Alt. Active Item</label>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <!-- /section:basics/sidebar.options -->\r\n");
      out.write("                        </div><!-- /.pull-left -->\r\n");
      out.write("                    </div><!-- /.ace-settings-box -->\r\n");
      out.write("                </div><!-- /.ace-settings-container -->\r\n");
      out.write("\r\n");
      out.write("                <!-- /section:settings.box -->\r\n");
      out.write("                 <div class=\"page-header\">\r\n");
      out.write("                    <h1>\r\n");
      out.write("                       机构管理\r\n");
      out.write("                        <small>\r\n");
      out.write("                            <i class=\"ace-icon fa fa-angle-double-right\"></i>\r\n");
      out.write("                           机构列表\r\n");
      out.write("                        </small>\r\n");
      out.write("                    </h1>\r\n");
      out.write("                </div><!-- /.page-header -->\r\n");
      out.write("\r\n");
      out.write("                 <div class=\"row\">\r\n");
      out.write("                    <div class=\"col-xs-12\">\r\n");
      out.write("                            <div class=\"col-sm-3\">\r\n");
      out.write("                                <div class=\"widget-box widget-color-green2\">\r\n");
      out.write("                                    <div class=\"widget-header\">\r\n");
      out.write("                                        <h4 class=\"widget-title lighter smaller\">\r\n");
      out.write("                                            <i class=\"ace-icon fa fa-sitemap blue\"></i>\r\n");
      out.write("                                            部门\r\n");
      out.write("                                        </h4>\r\n");
      out.write("                                    </div>\r\n");
      out.write("\r\n");
      out.write("                                    <div class=\"widget-body\">\r\n");
      out.write("                                        <div class=\"widget-main padding-8\" style=\"height: 65%;overflow: auto\">\r\n");
      out.write("                                                <div id=\"deptTree\"></div>\r\n");
      out.write("                                        </div>\r\n");
      out.write("                                    </div>\r\n");
      out.write("                                </div><!--dget-box -->\r\n");
      out.write("                            </div><!-- /.col -->\r\n");
      out.write("                        <div class=\"col-sm-9\">\r\n");
      out.write("                            <div class=\"input-group col-sm-4 pull-left\">\r\n");
      out.write("                                <input type=\"text\" class=\"form-control search-query\" placeholder=\"输入机构名称\" name=\"name\" id=\"orgname\"/>\r\n");
      out.write("                                <span class=\"input-group-btn\">\r\n");
      out.write("\t\t\t\t\t\t\t\t<button type=\"button\" class=\"btn btn-purple btn-sm\" id=\"search\">\r\n");
      out.write("\t\t\t\t\t\t\t\t<span class=\"ace-icon fa fa-search icon-on-right bigger-110\"></span>\r\n");
      out.write("\t\t\t\t\t\t\t\t搜索\r\n");
      out.write("\t\t\t\t\t\t\t\t</button>\r\n");
      out.write("\t\t\t\t\t\t\t\t</span>\r\n");
      out.write("                            </div>\r\n");
      out.write("\r\n");
      out.write("                            <div class=\"btn-group pull-right\">\r\n");
      out.write("                                <button class=\"btn btn-white btn-primary\" onclick=\"add()\"><i class=\"fa fa-plus-circle bigger-110 blue\"></i> 添加</button>\r\n");
      out.write("                                <button class=\"btn btn-danger btn-white\" data-type=\"getCheckData\" id=\"dels\"><i class=\"fa fa-trash  bigger-110 pink\"></i> 删除</button>\r\n");
      out.write("                                <button class=\"btn btn-success btn-white btn-refresh\"><i class=\"fa fa-refresh  bigger-110 green\"></i> 刷新</button>\r\n");
      out.write("                                <button class=\"btn btn-warning btn-white\"><i class=\"fa fa-file-excel-o  bigger-110 orange\"></i> 导出</button>\r\n");
      out.write("                                <button class=\"btn btn-warning btn-white\" onclick=\"addOrgUser()\"><i class=\"fa fa-file-excel-o  bigger-110 orange\"></i> 选人</button>\r\n");
      out.write("                            </div>\r\n");
      out.write("                            <div class=\"layui-row\">\r\n");
      out.write("                              <table  class=\"layui-table\" lay-data=\"{id:'dataGrid',height:'full-250', url:'");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/dataGrid',loading:true,page:true,limits: [30,60,90,150,300,500,1000]}\" lay-filter=\"test\">\r\n");
      out.write("                                    <thead>\r\n");
      out.write("                                    <tr>\r\n");
      out.write("                                       <th lay-data=\"{checkbox:true, fixed: true}\"></th>\r\n");
      out.write("                                        <th lay-data=\"{field:'id', width:80, sort: true}\">ID</th>\r\n");
      out.write("                                        <th lay-data=\"{field:'name', width:80}\">机构部门</th>\r\n");
      out.write("                                        <th lay-data=\"{field:'number', width:80}\">机构码</th>\r\n");
      out.write("                                        <th lay-data=\"{field:'parentName', width:100}\">上级机构部门</th>\r\n");
      out.write("                                        <th lay-data=\"{field:'description', width:130}\">描述</th>\r\n");
      out.write("                                        <th lay-data=\"{fixed: 'right', width:120, align:'center', toolbar: '#barDemo'}\">操作</th>\r\n");
      out.write("                                    </tr>\r\n");
      out.write("                                    </thead>\r\n");
      out.write("                                </table>\r\n");
      out.write("                                <script type=\"text/html\" id=\"barDemo\">\r\n");
      out.write("                                    <div class=\"btn-group\">\r\n");
      out.write("                                        <button class=\"btn btn-white btn-success btn-xs\"  title=\"查看\" lay-event=\"detail\"><i class=\"fa fa-search\"></i></button>\r\n");
      out.write("                                        <button class=\"btn btn-white btn-primary btn-xs\" title=\"编辑\" lay-event=\"edit\"><i class=\"fa fa-edit\"></i></button>\r\n");
      out.write("                                        <button class=\"btn btn-white btn-danger btn-xs\" title=\"删除\" lay-event=\"del\"><i class=\"fa fa-trash-o\"></i></button>\r\n");
      out.write("                                    </div>\r\n");
      out.write("                                </script>\r\n");
      out.write("                          \r\n");
      out.write("                            </div>\r\n");
      out.write("                        </div><!--/.col-->\r\n");
      out.write("                        </div>\r\n");
      out.write("                        <!-- PAGE CONTENT ENDS -->\r\n");
      out.write("                    </div><!-- /.col -->\r\n");
      out.write("                </div><!-- /.row -->\r\n");
      out.write("            </div><!-- /.page-content -->\r\n");
      out.write("        </div>\r\n");
      out.write("    </div><!-- /.main-content -->\r\n");
      out.write("\r\n");
      out.write("    <!-- <div class=\"footer\">\r\n");
      out.write("        <div class=\"footer-inner\">\r\n");
      out.write("            #section:basics/footer\r\n");
      out.write("            <div class=\"footer-content\">\r\n");
      out.write("\t\t\t\t\t\t<span class=\"bigger-120\">\r\n");
      out.write("\t\t\t\t\t\t\t<span class=\"blue bolder\">昆明</span>\r\n");
      out.write("\t\t\t\t\t\t\t昊蝶科技有限公司\r\n");
      out.write("\t\t\t\t\t\t</span>\r\n");
      out.write("\r\n");
      out.write("            </div>\r\n");
      out.write("\r\n");
      out.write("            /section:basics/footer\r\n");
      out.write("        </div>\r\n");
      out.write("    </div> -->\r\n");
      out.write("\r\n");
      out.write("    <a href=\"#\" id=\"btn-scroll-up\" class=\"btn-scroll-up btn btn-sm btn-inverse\">\r\n");
      out.write("        <i class=\"ace-icon fa fa-angle-double-up icon-only bigger-110\"></i>\r\n");
      out.write("    </a>\r\n");
      out.write("</div><!-- /.main-container -->\r\n");
      out.write("\r\n");
      out.write("<!-- basic scripts -->\r\n");
      out.write("\r\n");
      out.write("<!--[if !IE]> -->\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("    window.jQuery || document.write(\"<script src='");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery.js'>\"+\"<\"+\"/script>\");\r\n");
      out.write("</script>\r\n");
      out.write("\r\n");
      out.write("<!-- <![endif]-->\r\n");
      out.write("\r\n");
      out.write("<!--[if IE]>\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("window.jQuery || document.write(\"<script src='");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery1x.js'>\"+\"<\"+\"/script>\");\r\n");
      out.write("</script>\r\n");
      out.write("<![endif]-->\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("    if('ontouchstart' in document.documentElement) document.write(\"<script src='");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery.mobile.custom.js'>\"+\"<\"+\"/script>\");\r\n");
      out.write("</script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/bootstrap.js\"></script>\r\n");
      out.write("\r\n");
      out.write("<!-- page specific plugin scripts -->\r\n");
      out.write("\r\n");
      out.write("<!--[if lte IE 8]>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/excanvas.js\"></script>\r\n");
      out.write("<![endif]-->\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery-ui.custom.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery.ui.touch-punch.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery.easypiechart.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/jquery.sparkline.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/flot/jquery.flot.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/flot/jquery.flot.pie.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/flot/jquery.flot.resize.js\"></script>\r\n");
      out.write("\r\n");
      out.write("<!-- ace scripts -->\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.scroller.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.colorpicker.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.fileinput.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.typeahead.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.wysiwyg.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.spinner.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.treeview.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.wizard.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.aside.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.ajax-content.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.touch-drag.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.sidebar.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.sidebar-scroll-1.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.submenu-hover.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.widget-box.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.settings.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.settings-rtl.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.settings-skin.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.widget-on-reload.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.searchbox-autocomplete.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/layer/layer.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/layui/layui.all.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/js/common.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/js/util.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/select2.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctx}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/plugin/jstree/dist/jstree.min.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/plugin/scroll/jquery.nicescroll.min.js\"></script>\r\n");
      out.write("<!-- the following scripts are used in demo only for onpage help and you don't need them -->\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/css/ace.onpage-help.css\" />\r\n");
      out.write("<link rel=\"stylesheet\" href=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/themes/sunburst.css\" />\r\n");
      out.write("\r\n");
      out.write("<script type=\"text/javascript\"> ace.vars['base'] = '");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin'; </script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/elements.onpage-help.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/assets/js/ace/ace.onpage-help.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/rainbow.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/language/generic.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/language/html.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/language/css.js\"></script>\r\n");
      out.write("<script src=\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${path}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/static/aceadmin/docs/assets/js/language/javascript.js\"></script>");
      out.write("<!-- inline scripts related to this page -->\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("function add() {\r\n");
      out.write("    layer.open({\r\n");
      out.write("        type: 2,\r\n");
      out.write("        title: '<i class=\"fa fa-plus-circle  blue\"></i>&nbsp;添加机构',\r\n");
      out.write("        shadeClose: false,\r\n");
      out.write("        shade: 0.8,\r\n");
      out.write("        maxmin: true,\r\n");
      out.write("        area: ['90%', '90%'],\r\n");
      out.write("        content: '");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/addPage' //iframe的url\r\n");
      out.write("    });\r\n");
      out.write("}\r\n");
      out.write("function addOrgUser() {\r\n");
      out.write("    layer.open({\r\n");
      out.write("        type: 2,\r\n");
      out.write("        title: '<i class=\"fa fa-plus-circle  blue\"></i>&nbsp;选人',\r\n");
      out.write("        shadeClose: false,\r\n");
      out.write("        shade: 0.8,\r\n");
      out.write("        maxmin: true,\r\n");
      out.write("        area: ['90%', '90%'],\r\n");
      out.write("        content: '");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/jsp/org/userOrgCheckbox.jsp' //iframe的url\r\n");
      out.write("    });\r\n");
      out.write("}\r\n");
      out.write("    jQuery(function($) {\r\n");
      out.write("\r\n");
      out.write("        function del(id){\r\n");
      out.write("            $.ajax({\r\n");
      out.write("                url:'");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/delete',\r\n");
      out.write("                data:{\"id\":id},\r\n");
      out.write("                dataType:'json',\r\n");
      out.write("                type:'post',\r\n");
      out.write("                success:function(data){\r\n");
      out.write("                        layer.msg(\"删除成功！\",{icon:1,time:1000});\r\n");
      out.write("                        //刷新jstree树\r\n");
      out.write("                        var tree =$(\"#deptTree\").jstree();\r\n");
      out.write("                        tree.refresh();\r\n");
      out.write("                    \r\n");
      out.write("                },\r\n");
      out.write("                error: function(XmlHttpRequest, textStatus, errorThrown){\r\n");
      out.write("                    layer.msg('删除失败!',{icon:5,time:1000});\r\n");
      out.write("                }\r\n");
      out.write("            })\r\n");
      out.write("        }\r\n");
      out.write("        layui.use('table', function(){\r\n");
      out.write("            var table = layui.table;\r\n");
      out.write("            //监听工具条\r\n");
      out.write("            table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter=\"对应的值\"\r\n");
      out.write("                var data = obj.data; //获得当前行数据\r\n");
      out.write("                var layEvent = obj.event; //获得 lay-event 对应的值\r\n");
      out.write("                var tr = obj.tr; //获得当前行 tr 的DOM对象\r\n");
      out.write("                if(layEvent === 'detail'){ //查看\r\n");
      out.write("                        layer.open({\r\n");
      out.write("                            type: 2,\r\n");
      out.write("                            title: '<i class=\"fa fa-search-plus  blue\"></i>&nbsp;查看机构信息',\r\n");
      out.write("                            shadeClose: true,\r\n");
      out.write("                            shade: 0.8,\r\n");
      out.write("                            maxmin: true,\r\n");
      out.write("                            area: ['50%', '90%'],\r\n");
      out.write("                            content:\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/viewPage?id=\"+data.id,\r\n");
      out.write("                        });\r\n");
      out.write("                } else if(layEvent === 'del'){ //删除\r\n");
      out.write("                    layer.confirm('真的删除'+data.orgname, function(index){\r\n");
      out.write("                        obj.del(); //删除对应行（tr）的DOM结构，并更新缓存\r\n");
      out.write("                        layer.close(index);\r\n");
      out.write("                        //向服务端发送删除指令\r\n");
      out.write("                        del(data.id);\r\n");
      out.write("                    });\r\n");
      out.write("                } else if(layEvent === 'edit'){ //编辑\r\n");
      out.write("                    layer.open({\r\n");
      out.write("                        type: 2,\r\n");
      out.write("                        title: '<i class=\"fa fa-edit  blue\"></i>&nbsp;编辑机构信息',\r\n");
      out.write("                        shadeClose: false,\r\n");
      out.write("                        shade: 0.8,\r\n");
      out.write("                        maxmin: true,\r\n");
      out.write("                        area: ['50%', '90%'],\r\n");
      out.write("                        content:\"");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/editPage?id=\"+data.id,\r\n");
      out.write("                    });\r\n");
      out.write("                }\r\n");
      out.write("            });\r\n");
      out.write("            var $ = layui.$, active = {\r\n");
      out.write("                getCheckData: function(){ //获取选中数据\r\n");
      out.write("                    var checkStatus = table.checkStatus('dataGrid')\r\n");
      out.write("                        ,data = checkStatus.data;\r\n");
      out.write("                    var ids=new  Array();\r\n");
      out.write("                    if(data.length==0){\r\n");
      out.write("                        layer.msg('请选择要删除的记录！')\r\n");
      out.write("                    }else{\r\n");
      out.write("                        layer.confirm('真的删除这'+data.length+'条记录吗?', function(index){\r\n");
      out.write("                            $.each(data,function(index,item){\r\n");
      out.write("                                ids.push(item.id);\r\n");
      out.write("                            });\r\n");
      out.write("                            $.ajax({\r\n");
      out.write("                                url:'");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/deleteBatch',\r\n");
      out.write("                                data:{\"ids\":ids.join(',')},\r\n");
      out.write("                                dataType:'json',\r\n");
      out.write("                                type:'post',\r\n");
      out.write("                                success:function(data){\r\n");
      out.write("                                        layer.msg(\"删除成功！\",{icon:1,time:1000});\r\n");
      out.write("                                      //刷新jstree树\r\n");
      out.write("                                        var tree =$(\"#deptTree\").jstree();\r\n");
      out.write("                                        tree.refresh();\r\n");
      out.write("                                        table.reload('dataGrid',{url:'");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/dataGrid'});\r\n");
      out.write("                                },\r\n");
      out.write("                                error: function(XmlHttpRequest, textStatus, errorThrown){\r\n");
      out.write("                                    layer.msg('删除失败!',{icon:5,time:1000});\r\n");
      out.write("                                }\r\n");
      out.write("                            })\r\n");
      out.write("                        })\r\n");
      out.write("                    }\r\n");
      out.write("                }\r\n");
      out.write("            };\r\n");
      out.write("\r\n");
      out.write("            $('#dels').on('click', function(){\r\n");
      out.write("                var type = $(this).data('type');\r\n");
      out.write("                active[type] ? active[type].call(this) : '';\r\n");
      out.write("            });\r\n");
      out.write("\r\n");
      out.write("            $('#search').on('click', function(){\r\n");
      out.write("                table.reload('dataGrid', {\r\n");
      out.write("                    where:{\r\n");
      out.write("                        name:$('#orgname').val(),\r\n");
      out.write("                    }\r\n");
      out.write("                });\r\n");
      out.write("            })\r\n");
      out.write("\r\n");
      out.write("            $('.btn-refresh').on('click',function(){\r\n");
      out.write("            \t var tree =$(\"#deptTree\").jstree();\r\n");
      out.write("                 tree.refresh();\r\n");
      out.write("                table.reload('dataGrid',{url:'");
      out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${ctxPath}", java.lang.String.class, (javax.servlet.jsp.PageContext)_jspx_page_context, null));
      out.write("/organization/dataGrid'});\r\n");
      out.write("            })\r\n");
      out.write("\r\n");
      out.write("        });\r\n");
      out.write("       \r\n");
      out.write("            var setting = {\r\n");
      out.write("                view: {\r\n");
      out.write("                    selectedMulti: false\r\n");
      out.write("                },\r\n");
      out.write("                check: {\r\n");
      out.write("                    enable: false\r\n");
      out.write("                },\r\n");
      out.write("                data: {\r\n");
      out.write("                    simpleData: {\r\n");
      out.write("                        enable: true\r\n");
      out.write("                    }\r\n");
      out.write("                }\r\n");
      out.write("            };\r\n");
      out.write("          \r\n");
      out.write("\r\n");
      out.write("            $('#deptTree').jstree({\r\n");
      out.write("                'core' : {\r\n");
      out.write("                    'data' : {\r\n");
      out.write("                        \"url\" : basePath+\"/organization/jstree\",\r\n");
      out.write("                        \"dataType\" : \"json\" // needed only if you do not supply JSON headers\r\n");
      out.write("                    }\r\n");
      out.write("                }\r\n");
      out.write("            }).bind('select_node.jstree', function(event,selected,node) {\r\n");
      out.write("                var orgid=selected.node.id;\r\n");
      out.write("                console.log(event)\r\n");
      out.write("                layui.use('table', function() {\r\n");
      out.write("                    var table = layui.table;\r\n");
      out.write("                    table.reload('dataGrid', {\r\n");
      out.write("                        url: '/organization/dataGrid'\r\n");
      out.write("                        , where: {\"id\": orgid}\r\n");
      out.write("                    });\r\n");
      out.write("                })\r\n");
      out.write("            });\r\n");
      out.write("            $(\"#deptTree\").parent().niceScroll({\r\n");
      out.write("                cursorcolor:\"#ccc\",\r\n");
      out.write("                cursoropacitymax:1,\r\n");
      out.write("                touchbehavior:false,\r\n");
      out.write("                cursorwidth:\"6px\",\r\n");
      out.write("                cursorborder:\"0\",\r\n");
      out.write("                cursorborderradius:\"5px\"\r\n");
      out.write("            });\r\n");
      out.write("          \t//设置菜单激活\r\n");
      out.write("            var href=basePath+\"/organization/index\";\r\n");
      out.write("            $('.nav.nav-list a').parent().removeClass('active');\r\n");
      out.write("            $('.nav.nav-list a').each(function () {\r\n");
      out.write("               if($(this).attr('href')==href){\r\n");
      out.write("            \t   var level= $(this).data('level');\r\n");
      out.write("                   $(this).parent().addClass('active');\r\n");
      out.write("                   if(level==2){\r\n");
      out.write("                       $(this).parent().parent().parent().addClass('active open');\r\n");
      out.write("                   }\r\n");
      out.write("                   if(level==3){\r\n");
      out.write("                       $(this).parent().parent().parent().addClass('active open');\r\n");
      out.write("                       $(this).parent().parent().parent().parent().parent().addClass('active open');\r\n");
      out.write("                   }\r\n");
      out.write("                   if(level==4){\r\n");
      out.write("                       $(this).parent().parent().parent().addClass('active open');\r\n");
      out.write("                       $(this).parent().parent().parent().parent().parent().addClass('active open');\r\n");
      out.write("                       $(this).parent().parent().parent().parent().parent().parent().parent().addClass('active open');\r\n");
      out.write("                   }\r\n");
      out.write("\r\n");
      out.write("               }\r\n");
      out.write("            })\r\n");
      out.write("\r\n");
      out.write("\r\n");
      out.write("    })\r\n");
      out.write("</script>\r\n");
      out.write("</body>\r\n");
      out.write("</html>\r\n");
      out.write("\r\n");
    } catch (java.lang.Throwable t) {
      if (!(t instanceof javax.servlet.jsp.SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          try {
            if (response.isCommitted()) {
              out.flush();
            } else {
              out.clearBuffer();
            }
          } catch (java.io.IOException e) {}
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }

  private boolean _jspx_meth_c_005fset_005f0(javax.servlet.jsp.PageContext _jspx_page_context)
          throws java.lang.Throwable {
    javax.servlet.jsp.PageContext pageContext = _jspx_page_context;
    javax.servlet.jsp.JspWriter out = _jspx_page_context.getOut();
    //  c:set
    org.apache.taglibs.standard.tag.rt.core.SetTag _jspx_th_c_005fset_005f0 = (org.apache.taglibs.standard.tag.rt.core.SetTag) _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.get(org.apache.taglibs.standard.tag.rt.core.SetTag.class);
    _jspx_th_c_005fset_005f0.setPageContext(_jspx_page_context);
    _jspx_th_c_005fset_005f0.setParent(null);
    // /common/global.jsp(6,0) name = var type = java.lang.String reqTime = false required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f0.setVar("ctxPath");
    // /common/global.jsp(6,0) name = value type = javax.el.ValueExpression reqTime = true required = false fragment = false deferredValue = true expectedTypeName = java.lang.Object deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f0.setValue(new org.apache.jasper.el.JspValueExpression("/common/global.jsp(6,0) '${pageContext.request.contextPath}'",_jsp_getExpressionFactory().createValueExpression(_jspx_page_context.getELContext(),"${pageContext.request.contextPath}",java.lang.Object.class)).getValue(_jspx_page_context.getELContext()));
    int _jspx_eval_c_005fset_005f0 = _jspx_th_c_005fset_005f0.doStartTag();
    if (_jspx_th_c_005fset_005f0.doEndTag() == javax.servlet.jsp.tagext.Tag.SKIP_PAGE) {
      _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f0);
      return true;
    }
    _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f0);
    return false;
  }

  private boolean _jspx_meth_c_005fset_005f1(javax.servlet.jsp.PageContext _jspx_page_context)
          throws java.lang.Throwable {
    javax.servlet.jsp.PageContext pageContext = _jspx_page_context;
    javax.servlet.jsp.JspWriter out = _jspx_page_context.getOut();
    //  c:set
    org.apache.taglibs.standard.tag.rt.core.SetTag _jspx_th_c_005fset_005f1 = (org.apache.taglibs.standard.tag.rt.core.SetTag) _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.get(org.apache.taglibs.standard.tag.rt.core.SetTag.class);
    _jspx_th_c_005fset_005f1.setPageContext(_jspx_page_context);
    _jspx_th_c_005fset_005f1.setParent(null);
    // /common/global.jsp(7,0) name = var type = java.lang.String reqTime = false required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f1.setVar("path");
    // /common/global.jsp(7,0) name = value type = javax.el.ValueExpression reqTime = true required = false fragment = false deferredValue = true expectedTypeName = java.lang.Object deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f1.setValue(new org.apache.jasper.el.JspValueExpression("/common/global.jsp(7,0) '${ctxPath}'",_jsp_getExpressionFactory().createValueExpression(_jspx_page_context.getELContext(),"${ctxPath}",java.lang.Object.class)).getValue(_jspx_page_context.getELContext()));
    int _jspx_eval_c_005fset_005f1 = _jspx_th_c_005fset_005f1.doStartTag();
    if (_jspx_th_c_005fset_005f1.doEndTag() == javax.servlet.jsp.tagext.Tag.SKIP_PAGE) {
      _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f1);
      return true;
    }
    _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f1);
    return false;
  }

  private boolean _jspx_meth_c_005fset_005f2(javax.servlet.jsp.PageContext _jspx_page_context)
          throws java.lang.Throwable {
    javax.servlet.jsp.PageContext pageContext = _jspx_page_context;
    javax.servlet.jsp.JspWriter out = _jspx_page_context.getOut();
    //  c:set
    org.apache.taglibs.standard.tag.rt.core.SetTag _jspx_th_c_005fset_005f2 = (org.apache.taglibs.standard.tag.rt.core.SetTag) _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.get(org.apache.taglibs.standard.tag.rt.core.SetTag.class);
    _jspx_th_c_005fset_005f2.setPageContext(_jspx_page_context);
    _jspx_th_c_005fset_005f2.setParent(null);
    // /common/global.jsp(8,0) name = var type = java.lang.String reqTime = false required = false fragment = false deferredValue = false expectedTypeName = null deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f2.setVar("staticPath");
    // /common/global.jsp(8,0) name = value type = javax.el.ValueExpression reqTime = true required = false fragment = false deferredValue = true expectedTypeName = java.lang.Object deferredMethod = false methodSignature = null
    _jspx_th_c_005fset_005f2.setValue(new org.apache.jasper.el.JspValueExpression("/common/global.jsp(8,0) '${ctxPath}'",_jsp_getExpressionFactory().createValueExpression(_jspx_page_context.getELContext(),"${ctxPath}",java.lang.Object.class)).getValue(_jspx_page_context.getELContext()));
    int _jspx_eval_c_005fset_005f2 = _jspx_th_c_005fset_005f2.doStartTag();
    if (_jspx_th_c_005fset_005f2.doEndTag() == javax.servlet.jsp.tagext.Tag.SKIP_PAGE) {
      _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f2);
      return true;
    }
    _005fjspx_005ftagPool_005fc_005fset_0026_005fvar_005fvalue_005fnobody.reuse(_jspx_th_c_005fset_005f2);
    return false;
  }
}
