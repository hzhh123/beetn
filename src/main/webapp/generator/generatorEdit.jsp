...
<div class="layui-container-fluid" style="margin: 15px 10px;">
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="name" value="${codeBeanProperties.get(0).tableName}"
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="description" id="description" value="${codeBeanProperties.get(0).description}"
                           placeholder="请输入。。"
    ...
    <form class="layui-form" id="form2">
        <table class="layui-table" lay-even="" id="table">
            ...
            <tbody>
            <c:forEach items="${codeBeanProperties}" var="codeBeanProperty">
                <tr>
                    <td><input type="text" name="field" class="layui-input"
                               placeholder="请输入列名" value="${codeBeanProperty.field}"></td>
                    <td><select name="type" lay-filter="aihao">
                        <option value="" <c:if test="${codeBeanProperty.type==''}">selected</c:if>></option>
                        <option value="int" <c:if test="${codeBeanProperty.type=='int'}">selected</c:if>>int</option>
                        <option value="varchar" <c:if test="${codeBeanProperty.type=='varchar'}">selected</c:if>>varchar</option>
                        <option value="varchar" <c:if test="${codeBeanProperty.type=='text'}">selected</c:if>>text</option>
                        <option value="bigint" <c:if test="${codeBeanProperty.type=='bigint'}">selected</c:if>>bigint</option>
                        <option value="tinyint" <c:if test="${codeBeanProperty.type=='tinyint'}">selected</c:if>>tinyint</option>
                        <option value="date" <c:if test="${codeBeanProperty.type=='date'}">selected</c:if>>date</option>
                        <option value="datetime" <c:if test="${codeBeanProperty.type=='datetime'}">selected</c:if>>datetime</option>
                    </select></td>
                    <td><input type="text" name="length" class="layui-input" value="${codeBeanProperty.length}">
                    </td>
                    <td><input type="text" name="defaultValue" value="${codeBeanProperty.defaultValue}"
                               class="layui-input"></td>
                    <td>
                        <input type="checkbox" name="key" value="1" lay-skin="primary" title="" lay-filter="key" <c:if test="${codeBeanProperty.priKey=='PRI'}">checked</c:if>>
                    </td>
                    <td>
                        <input type="checkbox" name="isNull" value="1" lay-skin="primary" title="" class="isNull" <c:if test="${codeBeanProperty.isNull=='NO'}">checked</c:if>>
                    </td>
                    <td>
                        <input type="checkbox" name="extra" value="1" lay-skin="primary" title="" <c:if test="${codeBeanProperty.extra=='auto_increment'}">checked</c:if>>
                    </td>
                    <td><input type="text" name="comment" class="layui-input" value="${codeBeanProperty.comment}"></td>
                    <td>
                        <input type="checkbox" name="isTransient" value="1" lay-skin="primary" title=""  <c:if test="${codeBeanProperty.isTransient=='1'}">checked</c:if>>
                    </td>
                    <td><select name="showType" >
                        <option value="hidden" <c:if test="${codeBeanProperty.showType=='hidden'}">selected</c:if>>隐藏域</option>
                        <option value="text" <c:if test="${codeBeanProperty.showType=='text'}">selected</c:if>>文本</option>
                        <option value="textarea" <c:if test="${codeBeanProperty.showType=='texarea'}">selected</c:if>>大文本</option>
                        <option value="date" <c:if test="${codeBeanProperty.showType=='date'}">selected</c:if>>日期</option>
                        <option value="datetime" <c:if test="${codeBeanProperty.showType=='datetime'}">selected</c:if>>日期时间</option>
                        <option value="radio" <c:if test="${codeBeanProperty.showType=='radio'}">selected</c:if>>单选</option>
                        <option value="checkbox" <c:if test="${codeBeanProperty.showType=='checkbox'}">selected</c:if>>多选</option>
                        <option value="number" <c:if test="${codeBeanProperty.showType=='number'}">selected</c:if>>数字</option>
                        <option value="email" <c:if test="${codeBeanProperty.showType=='email'}">selected</c:if>>邮箱</option>
                        <option value="tel" <c:if test="${codeBeanProperty.showType=='tel'}">selected</c:if>>电话</option>
                        <option value="select" <c:if test="${codeBeanProperty.showType=='select'}">selected</c:if>>下拉选择</option>
                        <option value="select2" <c:if test="${codeBeanProperty.showType=='select2'}">selected</c:if>>select2</option>
                    </select></td>
                    <td>
                        <input type="checkbox" name="isQuery" value="1" lay-skin="primary" title="" <c:if test="${codeBeanProperty.isQuery=='1'}">checked</c:if>>
                    </td>
                    <td>
                        <select name="valid">
                            <option value="" <c:if test="${codeBeanProperty.valid==''|| codeBeanProperty.valid==null}">selected</c:if>></option>
                            <option value="required:true" <c:if test="${codeBeanProperty.valid=='required:true'}">selected</c:if>>必填</option>
                        </select>
                    </td>
                    <td>
...
<script type="text/javascript">
    layui.use(['form'], function() {
        ...
        $('[name=name]').change(function(){
            var value='${codeBeanProperties.get(0).tableName}';
        ...
        $('#save').click(function(){
            ...
            if(tableName==""){
                ...
                var data={"id":id,"tableName":tableName,"description":description,"fields":fields.join(","),"types":types.join(","),"lengths":lengths.join(","),"defaultValues":defaultValues.join(","),
                    "keys":keys.join(","),"isNulls":isNulls.join(","),"extras":extras.join(","),"comments":comments.join(","),"isQuerys":isQuerys.join(","),"isTransients":isTransients.join(","),"showTypes":showTypes.join(","),
                     "valids":valids.join(",")};
                $.ajax({
