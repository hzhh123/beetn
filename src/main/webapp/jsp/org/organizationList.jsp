                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="col-sm-9">
                            ...
                            <div class="btn-group pull-right">
                                ...
                                <button class="btn btn-warning btn-white" onclick="addOrgUser()"><i class="fa fa-file-excel-o  bigger-110 orange"></i> 选人</button>
                            </div>
                            <div class="layui-row">
                              <table  class="layui-table" lay-data="{id:'dataGrid',height:'full-250', url:'${ctxPath}/organization/dataGrid',loading:true,page:true,limits: [30,60,90,150,300,500,1000]}" lay-filter="test">
                                    <thead>
                                    <tr>
                                       <th lay-data="{checkbox:true, fixed: true}"></th>
                                        <th lay-data="{field:'id', width:80, sort: true}">ID</th>
                                        <th lay-data="{field:'name', width:80}">机构部门</th>
                                        <th lay-data="{field:'number', width:80}">机构码</th>
...
function add() {
    layer.open({
        ...
        maxmin: true,
        area: ['90%', '90%'],
        content: '${ctxPath}/organization/addPage' //iframe的url
    });
}
function addOrgUser() {
    layer.open({
        type: 2,
        title: '<i class="fa fa-plus-circle  blue"></i>&nbsp;选人',
        shadeClose: false,
        shade: 0.8,
        maxmin: true,
        area: ['90%', '90%'],
        content: '${ctxPath}/jsp/org/userOrgCheckbox.jsp' //iframe的url
    });
}
    jQuery(function($) {
        ...
        function del(id){
            $.ajax({
                url:'${ctxPath}/organization/delete',
                data:{"id":id},
                dataType:'json',
                type:'post',
        ...
        layui.use('table', function(){
            ...
            table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'detail'){ //查看
                        layer.open({
                            ...
                            maxmin: true,
                            area: ['50%', '90%'],
                            content:"${ctxPath}/organization/viewPage?id="+data.id,
                        });
                    ...
                    layer.open({
                        ...
                        maxmin: true,
                        area: ['50%', '90%'],
                        content:"${ctxPath}/organization/editPage?id="+data.id,
                    });
                }
            });
            var $ = layui.$, active = {
                getCheckData: function(){ //获取选中数据
                    ...
                    if(data.length==0){
                        ...
                        layer.confirm('真的删除这'+data.length+'条记录吗?', function(index){
                            ...
                            $.ajax({
                                url:'${ctxPath}/organization/deleteBatch',
                                data:{"ids":ids.join(',')},
                                dataType:'json',
                                type:'post',
                                success:function(data){
                                      ...
                                      //刷新jstree树
                                        var tree =$("#deptTree").jstree();
                                        tree.refresh();
                                        table.reload('dataGrid',{url:'${ctxPath}/organization/dataGrid'});
                                },
                                error: function(XmlHttpRequest, textStatus, errorThrown){
                                    layer.msg('删除失败!',{icon:5,time:1000});
                                }
            ...
            $("#deptTree").parent().niceScroll({
          	...
          	//设置菜单激活
            var href=basePath+"/organization/index";
